...
function injectTerm(
  ...
  // Inject i18n translations if provided
  if (proposal.i18n) {
    ...
    for (const [locale, entry] of Object.entries(proposal.i18n)) {
      ...
      const i18nData: Record<string, { term: string; definition: string }> =
        JSON.parse(readFileSync(i18nPath, "utf-8"));
      i18nData[proposal.id] = {
        term: entry.term,
        definition: proposal.definition,
      };
      // Sort by key for consistency
      const sorted = Object.fromEntries(
        Object.entries(i18nData).sort(([a], [b]) => a.localeCompare(b)),
      );
      writeFileSync(i18nPath, JSON.stringify(sorted, null, 2) + "\n", "utf-8");
    }
  }
